    ...
    if ($user_id && $user_type == '1') {
        ...
        if ($rowcount == 0) {
            ...
            while ($row = $sql->fetch()) {
               ...
               if ($row['Attachment']){
                    $attachment = $main_path . '/' . $media_grievance_report_path . '/' . $row['Attachment'];
                } else {
                    $attachment = 'NA';
                }
                $kashtkar_sahmati[] = array("villageName" => $row['VillageName'], "villageCode" => $row['VillageCode'], "gataNo" => $row['GataNo'], "khataNo" => $row['KhataNo'], "area" => $row['Area'], "mobile" => $row['Mobile'], "kashtkarName" => $row['owner_name'], "ownerFather" => $row['OwnerFather'], "complain" => $row['Grievance'], "dateSubmitted" => date('d-m-Y', $row['DateCreated']), "status" => $status, "reportedByOsd" => $attachment, "remark" => $row['Remarks']);
            }

            //Creating JSON
            $data = array('status' => true, 'message' => 'Grivances', 'total_pages' => $total_pages, "grivancesInfo" => $kashtkar_sahmati);
        }
    } else {
        //Creating JSON
