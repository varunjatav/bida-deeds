...
$(document).ready(function () {
  ...
  // add new data popup
  $(".full-column").on("click", ".add_new_user", function () {
    $("#popup").load("popup/addNewUserDataPopup.php", function () {
      $("#popup").show();
  ...
  $(".full-column").on("click", ".edit_file", function () {
    ...
    $("#popup").load(
      "popup/editUserDataPopup.php?file_id=" + file_id,
      function () {
        $("#popup").show();
        $("body").css("overflow-y", "hidden");
        makeDragable(".popup-header", ".popup-wrap");
  ...
  $("#popup").on("click", "#add_user_data", function () {
    ...
    if (check > 0) {
      var idx = fldrequired_index_arr.indexOf(
        Math.min.apply(null, fldrequired_index_arr)
      );
      fldrequired_index_arr[idx];
      $("html, body").animate(
        {
          scrollTop:
            $(".fldrequired:eq(" + fldrequired_index_arr[idx] + ")").offset()
              .top - 100,
        },
        500,
        function () {
      ...
      $("#userfrm")
        ...
        .append(
          '<input type="hidden" name="action" value="add_user_data" autocomplete="off">'
        );
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
          '<input type="hidden" name="action_url" value="action/userDataAction" autocomplete="off">'
        );
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
          '<input type="hidden" name="action_btn_id" value="#add_user_data" autocomplete="off">'
        );
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
          '<input type="hidden" name="action_btn_name" value="Save" autocomplete="off">'
        );
      $("#frm")
        .find(".frm_hidden_data")
        .append(
      ...
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
          '<input type="hidden" name="after_success_redirect" value="" autocomplete="off">'
        );
      $("#userfrm").submit();
    }
  });
  ...
  $("#popup").on("click", "#edit_user_data", function () {
    ...
    if (check > 0) {
      ...
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
          '<input type="hidden" name="action" value="edit_user_data" autocomplete="off">'
        );
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
      ...
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
          '<input type="hidden" name="action_btn_id" value="#edit_user_data" autocomplete="off">'
        );
      $("#userfrm")
        .find(".frm_hidden_data")
        .append(
...
function validateMaxlength(place) {
  ...
  if (char_count >= 0) {
    if (char_count === 1 || char_count === 0) {
      $(".msg_txt")
        .eq(place)
        .text("* " + char_count + " character remaining.");
    } else {
      $(".msg_txt")
        .eq(place)
        .text("* " + char_count + " characters remaining.");
    }
  }
}
...
function showAppliedFilter() {
  ...
  $("#popup input").each(function () {
    if ($(this).val() && $(this).attr("type") === "text") {
      var filter_key_id = $(this)
        .closest(".tab1")
        .attr("id")
        .replace("stab_", "");
      var filter_key = $("#popup")
        .find(".filter-tabber")
        .find("#" + filter_key_id)
        .text();
      var filter_value = $(this).val();
      applied_filter += generateFilter(
        filter_key_id,
        filter_key,
        filter_value,
        $(this).attr("type"),
  ...
  $(".filter-nos")
    .text($(".filterchip").length + " filters applied")
    .removeClass("hide");
...
function exportMasterData(exportlist, export_type, type) {
  ...
  if (export_type === "excel") {
    $(".act_btn_ovrly").show();
    $(".export_kashtkar_sahmati").html(
      '<div class="medical-spinner" style="top:0; left:20%;" id="load"></div>'
    );
