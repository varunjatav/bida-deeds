        ...
        if ($(this).closest('.hero-box').find('.main-card').is(':visible') === true) {
            if ($(this).attr('id').length) {
                ...
                if (isNumeric(id)) {
                    ...
                    promise_arr.push(syncLandUpdateData(id, index));
                }
            }
        }
    });
...
//    });
    ...
    $('#popup').on('change', '.tree_id', function () {
        if ($(this).val()) {
            $('.dimention_number').val('');
            $('.dimention_amount').val('');
            $('#dimention_number').removeClass('hide');
            $('#dimention_number').find('input').addClass('fldrequired');
        } else {
            $('#dimention_number').addClass('hide');
            $('#dimention_number').find('input').removeClass('fldrequired');
            $('.dimention_amount').val('');
            $('.dimen_amt').addClass('hide');
        }
    });

    $('#popup').on('keyup', '.dimention_number', function () {
        ...
        if (dimention_val) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#dimention_amount').removeClass('hide');
                    $('.dimen_amt').html('').html(data);
                }
            });
        } else {
            $('.dimention_amount').val('');
...
function syncLandUpdateData(data_point, count) {
    var dataString = 'data_point=' + data_point + '&count=' + count + '&action=sync_landupdate_data';
    ...
    $.ajax({
        url: 'ajax/syncLandUpdateData.php',
        data: dataString,
        type: "POST",
        success: function (data) {
            //console.log(data);
