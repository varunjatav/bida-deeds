...
// add new data popup
    $('.full-column').on('click', '.add_new_data', function () {
        $('#popup').load('popup/addNewDataPopup.php', function () {
            $('#popup').show();
    ...
    $('.full-column').on('click', '.edit_rtk_1359_data', function () {
        var curr = $(this);
        var id = curr.attr('id');
        var dataString = 'id=' + id;

        $.ajax({
            url: 'ajax/get1359FasliEditData.php',
            data: dataString,
            type: "POST",
            beforeSend: function () {
            },
            success: function (data) {
                curr.closest('.rowDiv').html('').html(data);
            },
    ...
    $('.full-column').on('change', '.mahal_1359', function () {
        var curr = $(this);
        var village_code = curr.closest('.rowDiv').find('.village_code_class').val();
        var dataString = 'mahal_name=' + curr.val() + '&village_id=' + village_code;

        $.ajax({
            url: 'ajax/getKhataByMahal.php',
            data: dataString,
            type: "POST",
            beforeSend: function () {
            },
            success: function (data) {
                curr.closest('.rowDiv').find('.khata_1359').html(data);
            },
            error: function (xhr, status, error) {
                console.error('Error fetching new data:', error);
            }
    ...
    $('.full-column').on('change', '.khata_1359', function () {
        var curr = $(this);
        var village_code = curr.closest('.rowDiv').find('.village_code_class').val();
        var mahal_name = curr.closest('.rowDiv').find('.mahal_1359').val();
        var dataString = 'khata_no=' + curr.val() + '&village_id=' + village_code + '&mahal_name=' + mahal_name;

        $.ajax({
            url: 'ajax/getGataByKhata.php',
            data: dataString,
            type: "POST",
            beforeSend: function () {
            },
            success: function (data) {
                curr.closest('.rowDiv').find('.gata_1359').html(data);
            },
            error: function (xhr, status, error) {
                console.error('Error fetching new data:', error);
            }
    ...
    $('.full-column').on('click', '.save_rtk_mapping_data', function () {
        var curr = $(this);

        var id = curr.attr('id');
        var village_code = curr.closest('.rowDiv').find('.village_code_class').val();
        var mahal_name = curr.closest('.rowDiv').find('.mahal_1359').val();
        var gata = curr.closest('.rowDiv').find('.gata_1359').val();
        var khata = curr.closest('.rowDiv').find('.khata_1359').val();

        if (mahal_name == '' || khata == '' || gata == '') {
            $.confirm({
                title: "Alert",
                message: "All fields are mendatory for selection",
                buttons: {
                    "OK": {
                        "class": "curr_btn",
                        action: function () {
        ...
        var dataString = 'khata_no=' + khata + '&village_id=' + village_code + '&mahal_name=' + mahal_name + '&id=' + id + '&gata=' + gata + '&action=rtk_mapping_data';
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (response_data['status'] === '-1') {
                    ...
                    curr.closest('.rowDiv').find('.mahal_dpdn').html('').text(response_data['success_array']['mahal_name']);
                    curr.closest('.rowDiv').find('.khata_dpdn').html('').text(response_data['success_array']['khata']);
                    curr.closest('.rowDiv').find('.gata_dpdn').html('').text(response_data['success_array']['gata']);
                    //.closest('.rowDiv').find('.save_btn').html('').text('1359 fasli data added');
                    curr.unbind('click').removeClass("save_rtk_mapping_data save-btn").addClass("edit_rtk_1359_data edit-btn").text('Edit');
    ...
    $('#mahal').change(function () {
        var mahalCode = $(this).val();
        if (mahalCode !== "") {
            $.ajax({
                url: 'ajax/get1359KhataData.php',
                type: 'POST',
                data: {mahal_code: mahalCode},
                success: function (response) {
                    $('#village').html(response);
                },
                error: function () {
                    alert('Error occurred while fetching village data');
                }
            });
        } else {
            $('#village').html('<option value="">Khata 1359 ke anusar</option>');
        }
    });
    ...
    $('.full-column').on('click', '#showFilter', function () {
        var sorting_val = $('#sort_by').val();
        if ($('#saveFilter').html() === '') {
            var curr = $(this);
            curr.addClass('active');
            var dataString = '';
            $.ajax({
                url: "ajax/showRTKFasliLandDataFilter.php",
                type: "POST",
                data: dataString,
                cache: false,
                success: function (data) {
                    $('#popup').html(data).show();
                    $('#popup').find('.sorting_database').val(sorting_val);
                    makeDragable('.popup-header', '.popup-wrap');
                }
            });
        } else {
    ...
    $("#popup").on('submit', '#ffrm', function (e) {
        ...
        currentFilterData = $(this).serialize() + '&action=filter_applied';
        var formURL = "ajax/filterRTKFasliLandData.php";
        setFilterHTML();
        $.ajax({
            url: formURL,
            type: "POST",
    ...
    $(".dev_wrap").on('change', '#sort_by', function (e) {
        var curr = $(this);
        var offset = currentOffset;
        var pagelimit = $('#pagelimit').val();
        var cleanedFilterData = currentFilterData.replace(/(&)?sorting_database=[^&]*/, '');
        var dataString = 'sorting_database=' + curr.val() + '&' + cleanedFilterData + '&offset=' + offset + '&pagelimit=' + pagelimit;
        $.ajax({
            url: 'ajax/filterRTKFasliLandData.php',
            data: dataString,
            type: "POST",
            success: function (data) {
                $('#paginate-body').html(data);
            }
        });
    });
...
function loadMoreRTKFasliLandData(page) {
    var fd = new FormData();
    fd.append('action', '');
    ...
    fd.append('sorting_database', $('#sort_by').val());
    $('#main-body').html('<div class="medical-spinner" id="load"></div>');
    $.ajax({
        type: "POST",
        url: "ajax/loadMoreRTKFasliLandData.php",
        data: fd,
        contentType: false,
        processData: false,
        cache: false,
