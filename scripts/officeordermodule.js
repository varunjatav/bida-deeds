...
$(document).ready(function () {
    ...
    $('.full-column').on('click', '#showFilter', function () {
        if ($('#saveFilter').html() === '') {
            ...
            var dataString = '';
            $.ajax({
                url: "ajax/showOfficeOrderModule.php",
                type: "POST",
                data: dataString,
                cache: false,
    ...
    $("#popup").on('submit', '#ffrm', function (e) {
        ...
        var formURL = "ajax/filterOfficeOrderModule.php";
        setFilterHTML();
        $.ajax({
            url: formURL,
            type: "POST",
    ...
    $('.full-column').on('click', '.setPage', function () {
        ...
        loadMoreOfficeOrderModule(0);
    });

    $('.full-column').on('click', '.export_excel', function () {
        exportOfficeOrderModule('export', 'excel', $(this).attr('id'));
    });

    $('.full-column').on('click', '.export_pdf', function () {
        exportOfficeOrderModule('export', 'pdf', $(this).attr('id'));
    });

    $('.full-column').on('click', '.export_print', function () {
        exportOfficeOrderModule('export', 'print', $(this).attr('id'));
    });
    ...
    // add new file popup
    $('.dev_wrap').on('click', '.office_order', function () {
        $('#popup').load('popup/officeOrderModulePopup.php', function () {
            $('#popup').show();
    ...
    // ### save office order module ###
    $('#popup').on('click', '#add_office_order', function () {
        var check = 0;
        ...
        if (check > 0) {
            ...
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action" value="office_order" autocomplete="off">');
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action_url" value="action/lamsAction" autocomplete="off">');
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action_btn_id" value="#add_office_order" autocomplete="off">');
...
function loadMoreOfficeOrderModule(page) {
    var fd = new FormData();
    fd.append('action', '');
    ...
    $.ajax({
        type: "POST",
        url: "ajax/loadMoreOfficeOrderModule.php",
        data: fd,
        contentType: false,
        processData: false,
        cache: false,
...
function exportOfficeOrderModule(exportlist, export_type, type) {
    var column_arr = [];
    var column_head = [];
    var fd = new FormData();
    fd.append('action', '');
    ...
    if (export_type === 'excel') {
        ...
        var formUrl = "report/export_ofice_order_module.php";
    } else if (export_type === 'pdf') {
        $('.act_btn_ovrly').show();
        $('.export_pdf').html('<div class="medical-spinner" style="top:0; left:20%;" id="load"></div>');
        var formUrl = "pdf/export_ofice_order_module.php";
