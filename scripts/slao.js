...
$(document).ready(function () {
    ...
    $('.full-column').on('click', '.export_excel', function () {
        exportSlaoData('export', 'excel', $(this).attr('id'));
    });
    ...
    $(".col-wrapper").on('submit', '#exfrm', function (e) {
        ...
        postData.append('action', 'import_slao_data');
    ...
    $("#popup").on('submit', '#ffrm', function (e) {
        ...
        var formURL = "ajax/filterSlaoData.php";
        setFilterHTML();
        $.ajax({
            url: formURL,
            type: "POST",
    ...
    $('.full-column').on('click', '.delete_slao_data', function () {
        var curr = $(this);
        $.confirm({
            ...
            buttons: {
                "Yes": {
                    ...
                    action: function (e) {
                        ...
                        var id = curr.attr('id');
                        var dataString = 'id=' + id + '&action=delete_slao_data';
                        $.ajax({
                            url: "action/lamsAction.php",
                            type: "POST",
                            data: dataString,
            ...
            buttons: {
                Yes: {
                    ...
                    action: function (e) {
                        ...
                        var dataString = 'ids=' + ids + '&action=delete_bulk_slao_data';
                        $.ajax({
                            url: 'action/lamsAction.php',
                            type: "POST",
                            data: dataString,
...
function loadMoreSlaoData(page) {
    var fd = new FormData();
    fd.append('action', '');
...
function exportSlaoData(exportlist, export_type, type) {
    var column_arr = [];
    var column_head = [];
    var fd = new FormData();
    fd.append('action', '');
    ...
    if (export_type === 'excel') {
        ...
        var formUrl = "report/export_slao_report.php";
    } else if (export_type === 'pdf') {
        var formUrl = "pdf/export_slao_report.php";
