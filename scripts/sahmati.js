...
$(document).ready(function () {
    ...
    $('.full-column').on('click', '#showFilter', function () {
        if ($('#saveFilter').html() === '') {
            ...
            var dataString = '';
            $.ajax({
                url: "ajax/showKashtkarSahmati.php",
                type: "POST",
                data: dataString,
                cache: false,
                success: function (data) {
    ...
    $("#popup").on('submit', '#ffrm', function (e) {
        ...
        var formURL = "ajax/filterKashtkarSahmati.php";
        setFilterHTML();
        $.ajax({
            url: formURL,
            type: "POST",
...
function loadMoreKashtkarSahmati(page) {
    var fd = new FormData();
    fd.append('action', '');
    ...
    $.ajax({
        type: "POST",
        url: "ajax/loadMoreKashtkarSahmati.php",
        data: fd,
        contentType: false,
        processData: false,
        cache: false,
...
function exportKashtkarSahmati(exportlist, export_type, type) {
    var column_arr = [];
    var column_head = [];
    var fd = new FormData();
    fd.append('action', '');
    ...
    if (export_type === 'excel') {
        ...
        var formUrl = "report/export_kashtkar_sahmati.php";
    } else if (export_type === 'pdf') {
        var formUrl = "pdf/export_kashtkar_sahmati.php";
