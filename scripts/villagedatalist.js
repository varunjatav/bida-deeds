...
$(document).ready(function () {
    ...
    $('.full-column').on('click', '.edit_file', function () {
        ...
        $('#popup').load('popup/uploadParmanPatrPopup.php?file_id=' + file_id, function () {
            $('#popup').show();
    ...
    $('#popup').on('change', '.browsePic', function () {
        ...
        for (var i = 0; i <= fi.files.length - 1; i++) {
            ...
            var arrayExtensions = ["pdf", "PDF", 'JPG', 'jpg', 'PNG', 'DOCX', 'docx', 'DOC', 'doc', 'jpeg', 'JPEG'];
        ...
        $("#office_order").removeClass('hide');
        $("#office_order").text(filecount + ' File selected');
    ...
    // edit file
    $('#popup').on('click', '#upload_perman_patr', function () {
        var check = 0;
        ...
        if (check > 0) {
            ...
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action" value="upload_perman_patr" autocomplete="off">');
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action_url" value="action/deedAction" autocomplete="off">');
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action_btn_id" value="#upload_perman_patr" autocomplete="off">');
    ...
    $('.full-column').on('click', '.setPage', function () {
        ...
        loadMoreVillageData(0);
    });

    $('.full-column').on('click', '.export_excel', function () {
        exportVillageData('export', 'excel', $(this).attr('id'));
    });

    $('.full-column').on('click', '.export_pdf', function () {
        exportVillageData('export', 'pdf', $(this).attr('id'));
    });

    $('.full-column').on('click', '.export_print', function () {
        exportVillageData('export', 'print', $(this).attr('id'));
    });
...
function loadMoreVillageData(page) {
    var fd = new FormData();
    fd.append('action', '');
    ...
    $.ajax({
        type: "POST",
        url: "ajax/loadMoreVillageData.php",
        data: fd,
        contentType: false,
        processData: false,
        cache: false,
...
function exportVillageData(exportlist, export_type, type) {
    var column_arr = [];
    var column_head = [];
    var fd = new FormData();
    fd.append('action', '');
...
function initializeDropdowns() {
    // Dropdown toggle for village selection
