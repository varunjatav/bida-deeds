...
$(document).ready(function () {
    ...
    $('.dev_wrap').on('click', '.edit_file', function () {
        ...
        $('#popup').load('popup/editMasterDataDetailsPopup.php?file_id=' + file_id, function () {
            $('#popup').show();
    ...
    // edit file
    $('#popup').on('click', '#edit_master_details', function () {
        var check = 0;
        ...
        if (check > 0) {
            ...
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action" value="edit_master_details" autocomplete="off">');
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action_url" value="action/deedAction" autocomplete="off">');
            $('#pfrm').find('.frm_hidden_data').append('<input type="hidden" name="action_btn_id" value="#edit_master_details" autocomplete="off">');
    ...
    $('.dev_wrap').on('click', '.tehsildar_validate', function (e) {
        var curr = $(this);
        var fileId = $('input[name="file_id"]').val();
        $.confirm({
            title: validateMessageConfirmation['title'] || 'पुष्टिकरण की जरूरत',
            message: validateMessageConfirmation['message'] || 'क्या आप वाकई इसे सत्यापित करना चाहते हैं?',
            buttons: {
                Yes: {
                    class: "curr_btn",
                    text: validateMessageConfirmation['yes'] || 'हाँ',
                    action: function () {
                        curr.unbind("click").click(function () {
                            return false;
                        });
                        $(".curr_btn").text("कृपया प्रतीक्षा करें...");
                        $('#frm').find('.frm_hidden_data').html('');
                        $('#frm').find('.frm_hidden_data').append('<input type="hidden" name="action" value="tehsildar_validate" autocomplete="off">');
                        $('#frm').find('.frm_hidden_data').append('<input type="hidden" name="action_url" value="action/deedAction" autocomplete="off">');
                        $('#frm').find('.frm_hidden_data').append('<input type="hidden" name="action_btn_id" value=".tehsildar_validate" autocomplete="off">');
                        $('#frm').find('.frm_hidden_data').append('<input type="hidden" name="action_btn_name" value="Validate" autocomplete="off">');
                        $('#frm').find('.frm_hidden_data').append('<input type="hidden" name="after_success_action" value="" autocomplete="off">');
                        $('#frm').find('.frm_hidden_data').append('<input type="hidden" name="after_success_redirect" value="masterdatadetails?id=' + fileId + '" autocomplete="off">');
                        $('#frm').submit();
                    }
                },
                No: {
                    class: "",
                    text: validateMessageConfirmation['no'] || 'नहीं',
                    action: function () {
                    }
                }
            }
    ...
    $('.full-column').on('click', '.setPage', function () {
        ...
        loadMoreMasterData(0);
    });

    $('.full-column').on('click', '.export_excel', function () {
        exportFasliLandData('export', 'excel', $(this).attr('id'));
    });

    $('.full-column').on('click', '.export_pdf', function () {
        exportFasliLandData('export', 'pdf', $(this).attr('id'));
    });

    $('.full-column').on('click', '.export_print', function () {
        exportFasliLandData('export', 'print', $(this).attr('id'));
    });
    ...
    $('.full-column').on('click', '#showFilter', function () {
        if ($('#saveFilter').html() === '') {
            ...
            var dataString = '';
            $.ajax({
                url: "ajax/showMasterDataFilter.php",
                type: "POST",
                data: dataString,
                cache: false,
                success: function (data) {
    ...
    $("#popup").on('submit', '#ffrm', function (e) {
        ...
        var formURL = "ajax/filterMasterData.php";
        setFilterHTML();
        $.ajax({
            url: formURL,
            type: "POST",
...
function loadMoreMasterData(page) {
    var fd = new FormData();
    fd.append('action', '');
    ...
    $.ajax({
        type: "POST",
        url: "ajax/loadMoreMasterData.php",
        data: fd,
        contentType: false,
        processData: false,
        cache: false,
...
function exportFasliLandData(exportlist, export_type, type) {
    var column_arr = [];
    var column_head = [];
    var fd = new FormData();
    fd.append('action', '');
    ...
    if (export_type === 'excel') {
        ...
        var formUrl = "report/export_master_data.php";
    } else if (export_type === 'pdf') {
        var formUrl = "pdf/export_master_data.php";
