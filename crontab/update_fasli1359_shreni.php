...
$village_query = $db->prepare("SELECT T1.ID, T1.fasali_ke_anusar_sreni
                                FROM lm_gata T1
                                WHERE T1.fasali_ke_anusar_sreni = ?
                                ");
$village_query->bindValue(1, '1(क)');
$village_query->execute();
if ($village_query->rowCount()) {
    while ($row = $village_query->fetch()) {
        $updt = $db->prepare("UPDATE lm_gata SET fasali_ke_anusar_sreni = ? WHERE ID = ?");
