...
elseif (isset($_POST['action']) && $_POST['action'] == 'upload_perman_patr') {
    ...
    try {
        ...
        $rakba = $_POST['rakba'];
        $vivran = isset($_POST['vivran_data']) ? $_POST['vivran_data'] : array();
        ...
        $timestamp = time();
        $created_by = $_SESSION['UserID'];
        $unique_code = generate_unique_id($db, 'lm_base_land_data', 'UniqueID', 1, 6); // Generate unique code per iteration

        $insrt1 = $db->prepare("INSERT INTO  lm_base_land_data  (UniqueID, VillageCode, MahalKaName, Shreni, KhataNo, KashtkarDarjStithi, DateCreated, CreatedBy) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $insrt1->bindParam(1, $unique_code);
        $insrt1->bindParam(2, $village_code);
        $insrt1->bindParam(3, $mahal_name);
        $insrt1->bindParam(4, $shreni);
        $insrt1->bindParam(5, $khata_no);
        $insrt1->bindParam(6, $kastkar_status);
        $insrt1->bindParam(7, $timestamp);
        $insrt1->bindParam(8, $created_by);
        $insrt1->execute();
        ...
        commit($db, '1359 fasli data added successfully', $db_response_data);
    } catch (\Exception $e) {
        if ($db->inTransaction()) {
            $db->rollback();
        }
        ...
        if (count_($village_gata_array['DATA']['village_detail'])) {
            ...
            if (count_($village_code_f)) {
                ...
                for ($i = 0; $i < $fasli_batch_size_count; $i++) {
                    $fasli_query = "INSERT INTO lm_api_1359_fasli_data (VillageCode, GataNo, KhataNo, Area, OwnerNo, Shreni, kashtkar_owner_no, kashtkar_owner_name, kashtkar_owner_father, kashtkar_area, SyncedOn, OwnerAddress, PlotSeqNumber, LandPosYear, isAnsh) VALUES";
                    ; //Prequery
